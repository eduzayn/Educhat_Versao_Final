...
import { insertUserTagSchema } from '../../../shared/schema';
import { z } from 'zod';
...
export function registerUserTagsRoutes(app: Express) {
  ...
  app.post('/api/user-tags', requirePermission('inbox:write'), async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Usuário não autenticado' });
  ...
  app.put('/api/user-tags/:id', requirePermission('inbox:write'), async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Usuário não autenticado' });
  ...
  app.delete('/api/user-tags/:id', requirePermission('inbox:write'), async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Usuário não autenticado' });
      ...
      if (errorMessage.includes('está sendo usada')) {
  ...
  app.get('/api/user-tags/stats', requirePermission('inbox:read'), async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Usuário não autenticado' });
  ...
  app.post('/api/user-tags/:tagId/contacts/:contactId', requirePermission('inbox:write'), async (req: AuthenticatedRequest, res: Response) => {
    try {
  ...
  app.delete('/api/user-tags/:tagId/contacts/:contactId', requirePermission('inbox:write'), async (req: AuthenticatedRequest, res: Response) => {
    try {
      const tagId = parseInt(req.params.tagId);
      const contactId = parseInt(req.params.contactId);
  ...
  app.get('/api/user-tags/contacts/:contactId', requirePermission('inbox:read'), async (req: AuthenticatedRequest, res: Response) => {
    try {
      const contactId = parseInt(req.params.contactId);
