// Consolidação dos arquivos de autenticação: auth.ts, sync-auth.ts, permissions.ts
...
// Interfaces e tipos para permissões
...
export function requirePermission(permissionName: string, extractContext?: (req: AuthenticatedRequest) => any) {
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    if (!req.user) {
      return res.status(401).json({ message: "Não autenticado" });
    }

    // Lógica simplificada de permissões - pode ser expandida conforme necessário
...
async function checkUserPermission(userId: number, permissionName: string): Promise<boolean> {
  try {
