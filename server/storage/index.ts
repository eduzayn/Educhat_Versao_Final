...
export class DatabaseStorage implements IStorage {
  ...
  async getConversations(limit?: number, offset?: number, filters?: { period?: string; channel?: string; user?: string; team?: string }) {
    return this.conversation.getConversations(limit, offset, filters);
  }
  ...
  async searchQuickReplies(options: {
    query: string;
    category?: string;
    type?: string;
    userId?: number;
  }): Promise<any[]> {
    const { query, category, type, userId } = options;
    
    // Buscar todas as respostas rápidas disponíveis
    const allReplies = await this.quickReply.getUserQuickReplies(userId || 0);
    ...
    // Filtrar por categoria e tipo se especificados
    let filteredReplies = allReplies.filter((reply: any) => {
      const matchesCategory = !category || category === 'all' || reply.category === category;
      const matchesType = !type || type === 'all' || reply.type === type;
      return matchesCategory && matchesType;
    });
    
    // Implementar busca inteligente com score de relevância
    const scoredReplies = filteredReplies.map((reply: any) => {
      let score = 0;
      ...
      const hasExactWordMatch = titleWords.some((word: string) => 
        word === searchTerm || word.startsWith(searchTerm)
      );
    ...
    return scoredReplies
      .filter((reply: any) => reply.searchScore > 0)
      .sort((a: any, b: any) => {
